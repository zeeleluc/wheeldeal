<div class="flex justify-center items-center min-h-[70vh] px-4">
    <div class="w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-lg p-10">
            ...
            <div class="space-y-5 mt-6">
                @if($alternativeDates && $alternativeDates->isNotEmpty())
                    <div class="relative">
                        <input type="number" wire:model="passengers" min="1"
                               ...
                               title="Change passengers to refresh alternative dates">
                    ...
                    <div class="mt-4 p-4 border border-yellow-300 bg-yellow-50 rounded-lg">
                        <h3 class="text-lg font-medium text-yellow-800 mb-2">No cars available, try alternative dates:</h3>
                        @foreach($alternativeDates as $range)
                            <button
                                    wire:click="selectAlternativeDates('{{ $range['start_date'] }}', '{{ $range['end_date'] }}')"
                                    class="w-full mb-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-lg shadow-md transition-all"
                            >
                ...
                @else
                    <input type="number" wire:model="passengers" min="1"
                           ...
                           class="w-full border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 focus:outline-none">
